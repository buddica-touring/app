...
function setupProperties() {
  PropertiesService.getScriptProperties().setProperties({
    'SUPABASE_URL': 'https://ggqugvyskyiblxiycpci.supabase.co',
...
// ============================================================
// メッセージID処理済み管理（-label: フィルタ禁止の代替手段）
// ★ 絶対ルール: GmailクエリにラベルEXCLUDE(-label:xxx)を入れてはいけない
//   → GmailAppはスレッド単位検索のため後続メール(CXL等)が永久に見えなくなる
//   → 処理済みはメッセージID単位でPropertiesServiceに記録する
// ============================================================
var NHA_PROCESSED_IDS_KEY = 'bt_processed_msg_ids';
var NHA_PROCESSED_IDS_MAX = 3000;
var NHA_PROCESSED_IDS_TTL_MS = 3 * 24 * 60 * 60 * 1000; // 3日

function getProcessedMsgIds_() {
  try {
    var raw = PropertiesService.getScriptProperties().getProperty(NHA_PROCESSED_IDS_KEY) || '{}';
    var map = JSON.parse(raw);
    // TTL超過エントリを除去
    var now = Date.now();
    var cleaned = {};
    Object.keys(map).forEach(function(k) { if (now - map[k] < NHA_PROCESSED_IDS_TTL_MS) cleaned[k] = map[k]; });
    return cleaned;
  } catch(e) { return {}; }
}

function saveProcessedMsgIds_(existing, newIds) {
  var now = Date.now();
  newIds.forEach(function(id) { existing[id] = now; });
  // サイズ上限: 古い順に削除
  var keys = Object.keys(existing).sort(function(a,b) { return existing[a] - existing[b]; });
  while (keys.length > NHA_PROCESSED_IDS_MAX) { delete existing[keys.shift()]; }
  try {
    PropertiesService.getScriptProperties().setProperty(NHA_PROCESSED_IDS_KEY, JSON.stringify(existing));
  } catch(e) { Logger.log('saveProcessedMsgIds_ error: ' + e.message); }
}
...
function processNewEmails() {
  ...
  // ★ -label: 禁止。newer_than:2d のみ使用し、処理済みはメッセージIDで管理
...
function processMessage_(message, dryRun) {
  ...
  if (isCancellation) {
    // キャンセル: DB存在チェック（札幌の予約はDBにあっても高松GASでは処理しない）
...
function isTakamatsuReservation_(res) {
  ...
  // 住所判定: 香川 → true, 北海道 → false
  if (/香川県|高松市|香川/.test(address)) return true;
  ...
  if (/高松|香川|豊見城|宜野湾|浦添|北谷/.test(delPlace + colPlace)) return true;
...
function parseOfficial_(body) {
  ...
  // ★ CDW / NOC / フル を正確に判定（else-if構造で上書き防止）
  var hasCDW = /免責補償制度\(CDW\)\s*:\s*あり/.test(body);
  var hasNOC = /レンタカー安心パック\s*:\s*あり/.test(body);
  var insurance = (hasCDW && hasNOC) ? 'フル' : hasNOC ? 'NOC' : hasCDW ? '免責' : 'なし';
  var optB = 0, optC = 0, optJ = 0;
  ...
  // ★ 飛行機便名を抽出（到着便名 / 出発便名）
  var arrFlightMatch = body.match(/飛行機便名[^】]*】\s*[\r\n]+\s*([A-Za-z][A-Za-z0-9]+)/);
  var flight = arrFlightMatch ? arrFlightMatch[1].trim() : '';
  // ★ visit_type / return_type を自動判定（DEL > PU, COL > BD）
  var hasDeliveryDel = /【デリバリー（お届け）】[\s\S]*?\n\s*希望する/.test(body);
  var hasAirportPU   = /【ご出発時送迎】[\s\S]*?\n\s*希望する/.test(body);
  var hasDeliveryCol = /【デリバリー（回収）】[\s\S]*?\n\s*希望する/.test(body);
  var hasAirportBD   = /【返却時の空港送迎】[\s\S]*?\n\s*希望する/.test(body);
  var visitType  = hasDeliveryDel ? 'DEL' : hasAirportPU ? 'PU' : '';
  var returnType = hasDeliveryCol ? 'COL' : hasAirportBD ? 'BD' : '';
  return {
    ...
    del_place: delPlace, col_place: colPlace,
    opt_b: optB, opt_c: optC, opt_j: optJ,
...
function reservationExists_(reservationId) {
  var rows = supabaseGet_('bt_reservations', 'id=eq.' + encodeURIComponent(reservationId) + '&select=id,status');
...
function deleteFromTasks_(reservationId) {
  return supabaseDelete_('bt_tasks', 'reservation_id=eq.' + encodeURIComponent(reservationId));
}
...
function sendSlackSuccess_(items) {
  ...
  MailApp.sendEmail(getSlackEmail_(), '✅ 高松店新規予約取込完了通知 ' + items.length + '件', lines.join('\n'));
...
function sendSlackFailure_(items) {
  ...
  MailApp.sendEmail(getSlackEmail_(), '❌ 高松店新規予約取込失敗通知 ' + items.length + '件', lines.join('\n'));
...
function sendSlackCancel_(items) {
  ...
  lines.push('');
  lines.push('合計: ' + items.length + '件');
  MailApp.sendEmail(getSlackEmail_(), '🔄 高松店予約キャンセル処理 ' + items.length + '件', lines.join('\n'));
...
function updateHeartbeat_(key, stats) {
  try {
    ...
    UrlFetchApp.fetch(getSupabaseUrl_() + '/rest/v1/bt_app_settings', options);
...
function checkUnknownSenders_() {
  ...
  // ★ -label: 禁止。メッセージIDで重複通知を防ぐ（alerted map で管理済み）
  var query = 'to:reserve@rent-buddica-touring.jp newer_than:2d';
  var threads;
  try {
